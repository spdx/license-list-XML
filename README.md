...
* **bi-weekly calls**: We use conference calls to make decisions on topics and issues that may be difficult to only discuss via email or via Github. Calls are every other Thursday at noon Eastern Time (US). Conference call info is sent prior to the calls via the mailing list. If you join the mailing list, you should get a recurring invite at the beginning of each calendar year. Meeting minutes for the calls are in the [SPDX meetings repo](https://github.com/spdx/meetings/tree/master/legal); historical meeting minutes can be found at http://wiki.spdx.org/
